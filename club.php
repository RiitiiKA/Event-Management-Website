...
.nav-scroller { position: relative; z-index: 2; height: 2.75rem;
overflow-y: hidden;
}

.nav-scroller .nav { display: flex;
flex-wrap: nowrap; padding-bottom: 1rem; margin-top: -1px;
...
box-shadow: 0px 0px 6px -4px rgba(0, 0, 0, 0.75); border-radius: 10px;
...
google.charts.load('45', { 'packages': ['corechart'] }); google.charts.setOnLoadCallback(drawChart);

function drawChart() { var data = [];
var options = {
title: 'Club Activity',
legend: { position: 'right', textStyle: { fontSize: 9 } }, hAxis: { title: 'Month', titleTextStyle: { color: '#333' } }, vAxis: { minValue: 0 },
height: 500,
width: 440,
};
<?php
for ($i = 0; $i < count($grdata) / 12; $i++) { echo "
data[$i] = google.visualization.arrayToDataTable([ ['Month ', 'Total Events ', 'Recruitments '],";
for ($j = $i * 12; $j < ($i + 1) * 12; $j++) { echo $grdata[$j];
}
...
?>
var gcharts = document.getElementsByClassName('chart'); for (var i = 0; i < gcharts.length; i++) {
var chart = new google.visualization.AreaChart(gcharts[i]); chart.draw(data[i], options);
}
}
</script>
...
<input class="form-control mr-sm-2" type="text" placeholder="Enter Club Name" aria-label="Search" id="myinput" name="clubsearch">
...
$searchval = $_POST['clubsearch']; if ($searchval == "") {
...
?>
<div class="modal fade" id="Recruitment" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
...
<input type="text" class="form-control" id="teammem-name" name="Team_member_names"

<p style="color:grey">Enter names separated by coma's</p>
</div>
<div class="mb-3"> Motivation:
...
<div class="mb-3"> Club/Chapter Description:
...
<div class="mb-3"> Date of Starting:
